...
## 16. B-14 品種詳細スポット都道府県フィルタ 仕様書（確定）

### 概要
品種詳細ページのスポット一覧に都道府県フィルタ・見頃ラベル・見頃順ソートを追加。
カレンダーから遷移した際は選択中の都道府県を自動引き継ぎ。

### 初期値の優先順位
1. `location.state.fromPref`（カレンダーから遷移時）
2. `sakura_wizard_geo` キャッシュがあれば現在地都道府県
3. 「全国」（フォールバック）

### ソート順
品種ごとの開花ウィンドウ（`getVarietyBloomWindow`）を各スポット座標で計算したdaysScoreで昇順ソート。
見頃中=0、これから=正の日数、散り=1000+経過日数。

### 見頃ラベル
🟢見頃 / 🟡もうすぐ / 🔴散り頃 / ⏳これから（off_seasonは非表示）

### 新規作成ファイル
- `src/utils/prefectureUtils.ts` — PREFS_LIST / PREF_COORDS / detectPrefecture / getCachedPrefecture

### 変更ファイル
- `src/components/SakuraCalendar.tsx` — prefectureUtils使用・navigate時にstate引き渡し
- `src/components/VarietyDetail.tsx` — 都道府県ドロップダウン・見頃ラベル・見頃ソート
- `src/App.css` — 新スタイル追加

---

## 17. B-15 カレンダー日付ベースリニューアル 仕様書（確定）

### 概要
`SakuraCalendar.tsx` のカレンダーを「旬（10日単位）」から「日付ベース」に全面リニューアル。

### UI構成

```
[1月][2月][3月]...[12月]  ← 月タブ（横スクロール）
━━━━━━━━━━━━━━━━━━━━━━━━━
[ ヒートマップ：日単位バー（横スクロール） ]
━━━━━━━━━━━━━━━━━━━━━━━━━
4月17日（木）← 選択中の日付ヘッダー
...
### 月タブ
- 1〜12月の12タブ、横スクロール可能
- アクティブな月をハイライト（ピンク系のアンダーライン）
- タップするとその月1日のセルに `scrollIntoView` でスクロール

### 日単位ヒートマップ
- 2026年1月1日〜12月31日の365日分
- セル幅11px・ギャップ1px（横スクロール前提）
- バー高さ: `Math.max(2, intensity * 44)px`
- 色分け:
  - 平日: ピンク系（`#FF85A1`）
  - 土曜: 青系（`#64b5f6`）
  - 日曜・祝日: 赤系（`#ef9a9a`）
  - 選択中: 濃い色 + outline
  - 今日: `#FF4488` + 上部ドットマーカー + outline
- 月初セルのみ「N月」ラベルを下部に表示
- `data-date="YYYY-MM-DD"` 属性でDOMを特定

### 日本の祝日（2026年）ハードコード
`JAPAN_HOLIDAYS_2026: Set<string>` として17祝日を定義（キーは `YYYY-MM-DD`）

### 日付→品種マッピング（dayMap）
```ts
const { dayMap } = useMemo(() => {
  // selectedPref → soDate → filteredVarieties
  // 各品種の bloomWindow を計算し、start〜end の各日付にプッシュ
  // fuyu品種は秋(10-11月)にも追加
}, [selectedPref])
```

### 選択日の品種リスト
- レア度順でグループ化（既存と同じUI）
- `nowInBloom`: 今日の bloom window 内かどうかで「今見頃」バッジ表示

### 初期表示
- 今日の日付を選択状態（`getTodayKey()`）
- 今日が 2026年以外の場合は `2026-04-01` にフォールバック
- マウント後に今日のセルを `scrollIntoView` でスクロール

### 削除したもの
- `PERIODS`, `getCurrentPeriodKey`, `periodOrd`, `buildPeriodMap`, `periodMap`, `maxCount(旧)`, `selectedPeriod`
- 旧カレンダーセル（`.calendar-cell`, `.calendar-cell__bar` 等）→ 新クラス（`.cal-day-cell*`）に置き換え
- 旧 `.calendar-period-header` → `.calendar-date-header` に変更

### 変更ファイル
- `src/components/SakuraCalendar.tsx` — 全面書き直し
- `src/App.css` — 月タブ・日セル・曜日色スタイル追加（旧カレンダースタイル置き換え）
- `src/components/SpotListCard.tsx` — `[SO-DEBUG]` デバッグコード削除
